<template>
  <div class="m-8">
    <h1 class="font-bold text-4xl">
      Rehab Listings in
      {{
        currentLocation !== null ? currentLocation : listings[0].address.region
      }}
    </h1>
    ...
    <footer class="flex justify-center border-gray-500 border-t-2">
      <ul>
        <li v-for="(city, id) in cities" :key="id" class="inline p-5">
          <a
            :href="`state/${city.slug_state_name}/city/${city.slug_city_name}`"
            >{{ city.city_name }}</a
          >
        </li>
      </ul>
